  ...
  try {
    rr.onBefore(async (params) => {
        ...
        const tx = Transaction.makeCreateTransaction(
          ...
          [
            Transaction.makeOutput(
              Transaction.makeEd25519Condition(eddsaPublicKey),
              amount
            ),
          ],
          eddsaPublicKey
      ...
      if (zencode.match(ASSET_AMOUNT_METADATA)) {
        const [amount, asset, metadata, publicKey] = zencode.paramsOf(
          ASSET_AMOUNT_METADATA
        );
        if (!input[asset]) {
          throw new Error("Asset not found");
      ...
      if (zencode.match(TRANSFER)) {
        ...
        const tx = Transaction.makeTransferTransaction(
          ...
          [Transaction.makeOutput(Transaction.makeEd25519Condition(recipient))],
          null
        );
        data.planetmint_transaction =
          Transaction.serializeTransactionIntoCanonicalString(tx);
      ...
      if (zencode.match(TRANSFER_AMOUNT)) {
        ...
        // Read UTXO
        const txs = await connection.listOutputs(senderPublicKey, false);
        ...
        const txsResolved = await Promise.all(
          txs.map(async (txOld) => {
            return {
              txOld,
              txDict:
                await connection.getTransaction<TransactionOperations.TRANSFER>(
                  txOld.transaction_id
                ),
            };
          })
        );
        ...
        const tx = Transaction.makeTransferTransaction(
          ...
          [
            Transaction.makeOutput(
              Transaction.makeEd25519Condition(senderPublicKey),
              (currentAmount - amount).toString(10)
            ),
            Transaction.makeOutput(
              Transaction.makeEd25519Condition(recipientPublicKey),
              amountStr
            ),
          ],
          null
    ...
    rr.onSuccess(async (params) => {
      ...
      if (zencode.match(SIGNATURE)) {
        const [planetmintTransactionName, publicKeyName] =
          zencode.paramsOf(SIGNATURE);
        const planetmintTransaction =
          result[planetmintTransactionName] || input[planetmintTransactionName];
        ...
        const tx = JSON.parse(
          planetmintTransaction
        ) as TransactionCommon<TransactionOperations>;
