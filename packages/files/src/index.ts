...
export default (req: Request, res: Response, next: NextFunction) => {
  ...
  rr.onBefore(async (params) => {
    ...
    if (zencode.match(LS)) {
      ...
      const allLs = (
        await Promise.all(
          params.map(async ([p, name]: string[]) => {
            const f = input[p] || p;
            validatePath(f);
            try {
      ...
      const allStats = await Promise.all(
        allLs.map(async ([name, p, current]) => {
  ...
  rr.onSuccess(async (params) => {
    ...
    if (zencode.match(DOWNLOAD)) {
      ...
      for (let i = 0; i < allPassOutputs.length; i += 2) {
        const file = result[allPassOutputs[i]] || input[allPassOutputs[i]];
        const folder =
          result[allPassOutputs[i + 1]] || input[allPassOutputs[i + 1]];
        ...
        if (!folder) {
          throw new Error(
            `[FILES] destination folder not defined ${Object.keys(input)}`
          );
        }
        ...
        try {
          ...
          next(e);
          throw new Error(
            `[FILES] Error sending the result to "${file}": ${e}`
          );
        }
      }
    }
    if (zencode.match(STORE_RESULT)) {
      ...
      for (let i = 0; i < allPassOutputs.length; i += 2) {
        const variable = result[allPassOutputs[i]] || input[allPassOutputs[i]];
        const file =
          result[allPassOutputs[i + 1]] || input[allPassOutputs[i + 1]];
